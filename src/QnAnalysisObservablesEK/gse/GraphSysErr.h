...
public:
  ...
  virtual void Print(Option_t* option="R") const
  {
    ...
    if (attr) {
      gROOT->IndentLevel();
      Printf(" [option: %3d line (c/s/w):%3d/%1d/%2d fill "
	     "(c/s):%3d/%4d marker (c/s/s):%3d/%2d/%f]",
	...
	Double_t y = GetYandError(i,false,false,quad,false,eyl,eyh,wyl,wyh);
	// Double_t y = GetY(i);
	TString s;
	s.Form(" %3d: %+8f %+8f %+8f -> %+8f %+8f %+8f",
  ...
  /** 
   * Clear stuff in graph 
   * 
   * @param option Can be a combination of 
   *
   * - COMMON  Remove all common systematic uncertainties 
   * - P2P     Remove all point-to-point systematic uncertainties 
   * - SYS     Remove all systematic uncertainties 
   * - KEY     Remove key-value pairs
   * - QUAL    Remove qualifiers 
   */
  void Clear(Option_t* option="")
  {
    TString opt(option);
    opt.ToUpper();
    Bool_t cmn = false, p2p = false, key = false, qua=false;
    if (opt.Contains("COMMON")) cmn = true;
    if (opt.Contains("P2P"))    p2p = true;
    if (opt.Contains("SYS"))    cmn = p2p = true;
    if (opt.Contains("KEY"))    key = true;
    if (opt.Contains("QUAL"))   qua = true;
    if (cmn)                fCommon     .Clear();
    if (p2p)                fPoint2Point.Clear();
    if (key && fMap)        fMap       ->Clear();
    if (qua && fQualifiers) fQualifiers->Clear();
  }  
  /** 
   ...
   *   - EAST/WEST         Without COMMON - put common east/west
   *   - XBASE=x           Without COMMON - center common syst. around X=x
   *   - YBASE=y           Without COMMON - center common syst. around Y=y
   *   - YBASE=@@x         Without COMMON - center common syst. around Y=g(x)
   *
   * If option neither COMMON, MIN, MAX, nor YBASE are given, then the
   * common errors are displayed near the middle of the Y range
   *
  ...
  /** 
   * Take the ratio @f$ r=n/d@f$ of two numbers @f$ n@f$ and @f$ d@f$
   * with associated errors @f$\delta n@f$ and @f$\delta d@f$,
   * respectively.  The error on ratio is returned in the last
   * argument, and is given by
   * 
   * @f[ \delta r=\sqrt{\delta^2n/n^2+\delta^2d/d^2} @f] 
   * 
   * @param n   Numerator @f$ n@f$
   * @param en  Error on numerator @f$\delta n@f$ 
   * @param d   Denominator @f$ d@f$
   * @param ed  Error on denominator @f$ \delta d@f$ 
   * @param er  On return, the error on the ratio @f$ \delta r@f$ 
   * 
   * @return The ration @f$ n/d@f$ 
   */
   ...
   * @param verb    If true, be verbose about it
   */
  Int_t Average(const TCollection* others, Bool_t sep=true, Bool_t verb=false)
  {
	...
	while ((other = static_cast<GraphSysErr*>(oNext())) && off < 64) {
	  ...
	  ULong64_t contrib = (ULong64_t(op->GetUniqueID() & mOf) << off);;		
	  mask |= contrib;
	  // Printf("Adding (%d & 0x3F) << %d = 0x%llx to mask -> 0x%llx (%d)",
	  //        op->GetUniqueID(), off, contrib, ULong64_t(mask), pId);
	  off  += dOf;
	  if (verb)
	    Printf("Mark %s/%s as used", other->GetTitle(), op->GetTitle());
	  op->SetBit(kOnlyWeightBit);	
	} // while other
	if (off >= 64) {
	  Warning("Average",
	...
	// Printf("Setting mp2p mask for %d to 0x%llx",pId,ULong64_t(mask));
	mp2p[pId] = mask;
	j++;
      } // while p2p
	...
	while ((p2p = static_cast<HolderP2P*>(oNextP())) && off < 64) {
	  ...
	  // in the result.
	  if (verb) Printf("Mark %s/%s for weights only", other->GetTitle(),
			   p2p->GetTitle());
	  ...
	  ULong64_t contrib = (ULong64_t(p2p->GetUniqueID() & mOf) << off);
	  //Printf("Adding (%d & 0x3F) << %d = 0x%llx to mask -> 0x%llx (%d)",
	  //       p2p->GetUniqueID(), off, contrib, ULong64_t(mask), pId);
	  *mask |= contrib;
	}
	off += dOf;
      } // while other
	...
	if (fret >= 0) {
	  ...
	  // if (verb) Printf("Exact # %2d Graph # %2d -> %f (-%f,+%f)",
	  //   i, j, y, wyl, wyh);
	}
	else {
	  ...
	  // if (verb) Printf("Inter # %2d Graph # %2d -> [%2d,%2d] -> "
	  //  "[%f (-%f,+%f),%f (-%f,+%f)] -> %f (-%f,+%f)",
	  //  i, j, ai1[j], ai2[j],
	  //  y1, wyl1, wyh1, y2, wyl2, wyh2, y, wyl, wyh);
	}
	// if (verb)
	// Printf("Point # %2d Graph # %2d @ %f -> %f (+%f,-%f)",
	// 	    i, j, xi, y, wyl, wyh);
	...
	j++;
      ...
      if (verb) Printf("Point # %2d @ %f -> %f", i, xi, newY);
	...
	while ((other = static_cast<GraphSysErr*>(oNext()))) {
	  ...
	  Int_t sid = ((rem >> dOf*k) & mOf);
	  // Printf("For graph %d and avg %d with mask=0x%llx get id "
	  //        "(0x%llx >> %d * %d) & 0x3f=%d",
	  //        k, tid, ULong64_t(mask), ULong64_t(rem), dOf, k, sid);
   ...
   * Average one or more graphs. 
   *
   * The resulting graph is the mean of the input graphs.
   * Uncertainties are calculated by linear sigma combinations.
   * 
   * @param others  List of graphs
   * @param sep     If true, then try to separate out point-to-point errors. 
   * @param verb    If true, be verbose about it
   */
  Int_t Average2(const TCollection* others, Bool_t sep=true, Bool_t verb=false)
  {
    Int_t bId = Average(others, sep, verb);
    Int_t no  = others->GetEntries();

    std::map<Int_t,std::vector<HolderP2P*>> p2p;
    for (auto c : fCommon) {
      HolderCommon* cc = static_cast<HolderCommon*>(c);
      Int_t         ci = cc->GetUniqueID();
      double        up = 0;
      double        dw = 0;
      
      for (auto o : *others) {
	GraphSysErr*  go =  static_cast<GraphSysErr*>(o);
	HolderCommon* hc =  go->FindCommon(go->FindId(cc->GetTitle()));
	Printf("%s -> %s", cc->GetTitle(), hc->GetTitle());
	up               += hc->GetYUp();
	dw               += hc->GetYDown();
      }
      SetSysError(ci,  dw/no, up/no);
    }
    for (auto p : fPoint2Point) {
      HolderP2P* pp = static_cast<HolderP2P*>(p);
      Int_t      pi = pp->GetUniqueID();
      p2p[pi].resize(no,0);

      Int_t io = 0;
      for (auto o : *others) {
	GraphSysErr*  go  = static_cast<GraphSysErr*>(o);
	Int_t         opi = go->FindId(pp->GetTitle());
	if (opi <= 0) {
	  Warning("Average2", "P2P %s not found in %s",
		  pp->GetTitle(), go->GetTitle());
	  continue;
	  io++;
	}
	HolderP2P*    hp  = go->FindP2P(opi);
	p2p[pi][io]       = hp;
	io++;
	// Printf("%2d for %2d -> %s",pi,io,hp->GetName());
      }
    }
	...
	Int_t      pi  = pm.first;
	Int_t      nn  = 0;
	HolderP2P* hp  = FindP2P(pi);
	
	for (auto o : *others) {
	  // Printf("%2d for %2d -> %p",pi,io,pm.second[io]);
	  GraphSysErr* go = static_cast<GraphSysErr*>(o);
	  HolderP2P*   ohp =  pm.second[io];
	  if (!ohp) {
	    io++;
	    continue;
	  }
	  Double_t ey = ohp->GetY(i);
	  if (hp->IsRelative()) ey /= go->GetY(i);
	  sum += ey;                 
	  io++;
	  nn++;
	}
	SetSysError(pi, i, GetSysErrorX(pi, i), sum / nn);
      }
    }
    return bId;
  }
		    ...
		    Short_t   last=-1)
  {
    ...
    if (!cmn && sumc) {
      TIter next(&fCommon);
      HolderCommon* cmn = 0;
      while ((cmn = static_cast<HolderCommon*>(next()))) {
	Bool_t   rel = cmn->IsRelative();
	Double_t eyl = cmn->GetYDown(rel ? sum : twdt);
	Double_t eyh = cmn->GetYUp  (rel ? sum : twdt);
	Double_t eym = (eyh+eyl)/2;
	Printf("Contribution from common %s (%f,%f): %f + %f -> %f",
	       cmn->GetTitle(), eyl, eyh, eym, TMath::Sqrt(err2),
	       TMath::Sqrt(err2+eym*eym));
	err2 += eym*eym;
      }
    }
			    ...
			    Double_t           fac=1)
  {
    ...
    Bool_t   propp2p = (cancc || denrel || cmax || cmin);
    Int_t    id      = 0;
    Int_t    notUsed = 0;
	...
	continue;
      ...
      Int_t        cId  = ret->DefineCommon(comN->GetTitle(),rel,
					    eyl, eyh, kBox);
      if (kVerbose & kRatio) {
	::Info("Ratio", "Defining new common from both sys.uncer: %s (-%f,%f)",
	       comN->GetTitle(), eyl, eyh);
	ret->Print("sys");
      }
    ...
    while ((p2pN = static_cast<HolderP2P*>(nextNP()))) {      
      ...
      Int_t      mask = 0;
      if (kVerbose & kRatio)
	::Info("","Id of denom %s -> %d (%p, %s, %s)", p2pN->GetTitle(), idD,
	       p2pD, same ? "same" : "diff",
	...
	p2pD->SetBit(kUsedBit);
      ...
      Int_t      pId  = ret->DeclarePoint2Point(p2pN->GetTitle(),rel,
						kBox);
      if (kVerbose & kRatio)
	::Info("","Adding p2p syst.uncer: %s based on numerator",
	       p2pN->GetTitle());
	...
	continue;
      ...
      Bool_t   cmn    = true;  // Include the common errors
      Bool_t   stat   = false; // Exclude the statistical errors
      Bool_t   quad   = true;  // Add in quadrature
      Bool_t   nosqrt = false; // Do not return squared error 
      Double_t sdEyl  = 0;
      Double_t sdEyh  = 0;
	...
	sdEyl = den->GetStatErrorDown(di);
	sdEyh = den->GetStatErrorUp  (di);
	di1   = di;
      }
      else {
	...
	Double_t seyl1 = den->GetStatErrorDown(di1);
	Double_t seyh1 = den->GetStatErrorUp  (di1);
	Double_t seyl2 = den->GetStatErrorDown(di2);
	Double_t seyh2 = den->GetStatErrorUp  (di2);
	...
	sdEyl       = seyl1 + daX * (seyl2 - seyl1);
	sdEyh       = seyh1 + daX * (seyh2 - seyh1);
      }
	...
	if (false && (kVerbose & kRatio))
	  ::Info("Ratio", "Stat error (%d) at %6.3f: "
		 "num=-%6.4f,+%6.4f  denom=-%6.4f,+%6.4f  ->  -%6.4f,+%6.4f",
		 cnt, x, snEyl, snEyh, sdEyl, sdEyh,
		 sEyl, sEyh);
      }
	...
	Double_t lfc  = (!crel ? (TMath::Abs(dY) > 1e-9 ? 1/dY : 0) : 1);
	if (kVerbose & kRatio)
	  Printf("Doing error %d from n=%d (%f +%f -%f) "
		 "d=%d (%f +%f -%f) lfc=%f",
		 rId, nId, nY, pnEyl, pnEyh, dId, dY, pdEyh, pdEyl, lfc);
	if (nId == 0) {
	  ...
	  if (denrel) {
	    ...
	    peyl = TMath::Min(pnEyl, pdEyl) * lfc;
	    peyh = TMath::Min(pnEyh, pdEyh) * lfc;
	    if (kVerbose & kRatio)
	      Printf("min err (min(%f%%,%f%%),min(%f%%,%f%%))=(%f%%,%f%%)",
		     pnEyl*100*lfc,pdEyl*100*lfc,
		     pnEyh*100*lfc,pdEyh*100*lfc,
		     peyl*100,peyh*100);
	  }
	  else if (cmax) {
	    peyl = TMath::Max(pnEyl, pdEyl) * lfc;
	    peyh = TMath::Max(pnEyh, pdEyh) * lfc;
	    if (kVerbose & kRatio)
	      Printf("max err (max(%f%%,%f%%),max(%f%%,%f%%))=(%f%%,%f%%)",
		     pnEyl*100*lfc,pdEyl*100*lfc,
		     pnEyh*100*lfc,pdEyh*100*lfc,
		     peyl*100,peyh*100);
	  }
	    ...
	    if (kVerbose & kRatio)
	      Printf("sqrt(|%8f^2 - %8f^2|)=%8f", pnEyl,pdEyl,peyl);
	  }
	} // Else
	if (kVerbose & kRatio)
  ...
  Bool_t Symmetrize2(GraphSysErr* other)
  {
    Int_t    n  = other->GetN();
    Double_t x1 = other->GetX(0);
    Double_t xn = other->GetX(n-1);
    Double_t e1 = other->GetErrorXLeft(0);
    Double_t en = other->GetErrorXRight(n-1);
    TList    lt;
    GraphSysErr* ref = other->Reflect();
    // Printf("Other reflected");
    // ref->Print("xy sys sum");
    lt.Add(other);
    lt.Add(ref);

    Int_t bid = Average(&lt,true,false);
    // Printf("Averaged");
    // Print("xy sys sum");
    ...
    for (Int_t i = m-1; i >= 0; i--) {
      Double_t x  = GetX(i);
      Double_t xl = GetErrorXLeft(i);
      Double_t xr = GetErrorXRight(i);

      if (x < x1-e1 || x > xn+en) {
	// Printf("Point %d at %f is outside [%f,%f]",i,x,x1-e1,xn+en);
	RemovePoint(i);
      }
    }
    if (GetN() != n) {
      Printf("Error while symmetrizing, had %d points, now %d",n,GetN());
      return false;
    }
    return true;
  }
   ...
   */
  Bool_t Symmetrize(GraphSysErr* other, Bool_t sep=false)
  {
    GraphSysErr*  cpy = new GraphSysErr(*other);

    // Declare "blob" systematic point-to-point error 
    Int_t         id  = -1;
    if (!sep)     id  = DeclarePoint2Point("Syst.unc..", false, kBox);
	...
	if (id > 0) SetSysError(id, cnt, 0, GetSysErrorY(id, j));
	      ...
	      Int_t         nxsign=0)
  {
    ...
    Int_t n = GetN();
    for (Int_t i = 0; i < n; i++) {
      ExportPoint(out, i, true, sysNames, nsign, nxsign);
      out << std::endl;
    }
    out << "*dataend:\n" 
	<< "# End of dataset\n" << std::endl;
		     ...
		     Int_t nxsign=0)
  {
    ...
    while ((oCmn = nextCmn())) {
      ...
      Double_t ecl1  = -1; 
      Double_t ech1  = -1;
      // ::Info("Export", "Looking at common systematic \"%s\"", oNme.Data());
      // --- Loop over data ------------------------------------------
...
#if 0
	  Printf("\"%s\" value for \"%s\" is -%f,+%f (was -%f,+%f)",
		 gse->GetName(), oNme.Data(), eclT, echT, ecl1, ech1);
#endif 
	  if (ecl1 < 0) {
	    // First time we get this error
	    // ::Info("Export", "Storing first value -%f,+%f", eclT, echT);
	    ecl1 = eclT;
	    ech1 = echT;
	    continue;
	  }
	    ...
	    if (((ecl1 == 0 || ecl1 > tol) && TMath::Abs(ecl1-eclT) > tol) ||
		((ech1 == 0 || ech1 > tol) && TMath::Abs(ech1-echT) > tol)) {
#if 0
	      ::Info("Export", "Common systematic \"%s\" is not the same "
		     "over all graphs (was -%f,+%f), got (-%f,+%f)",
		     oNme.Data(), ecl1, ech1, eclT, echT);
#endif 
	      found = false;
	    } // incompatible
	  } // ecl1 >= 0
    ...
    for (Int_t i = 0; i < nPoints; i++) {
      ...
      Bool_t one = true;
      while ((gse = static_cast<GraphSysErr*>(next()))) {
	gse->ExportPoint(out, i, one, alsoNme, nsign, nxsign);
	one = false;
      }
      out << std::endl;
    }
   ...
   * Export to JSON 
   */
  void ToJSON(std::ostream& out, bool single=true, const char* ind="")
  {
	
    const char* sind = ind;
    if (single) {
      out << ind << "{\n";
      ind = (std::string(ind)+"  ").c_str();
    }
    
    const char* sub = (std::string(ind)+"  ").c_str();
    const char* subsub = (std::string(sub)+"  ").c_str();
      
    out << ind << JSON::k(GetName()) << " {\n"
	<< sub << JSON::k("x-label") << "       "
	<< JSON::s(fXTitle) << ",\n"
	<< sub << JSON::k("y-label") << "       "
	<< JSON::s(fYTitle) << ",\n"
	<< sub << JSON::k("stat-relative") << " "
	<< (fStatRelative ? "true" : "false") << ",\n"
	<< sub << JSON::k("counter") << "       " << fCounter << ",\n"
	<< sub << JSON::k("data-attr") << " {\n";
    JSON::outatt(out,fDataOption, GetTitle(),
		 this, this, this, 
		 // dynamic_cast<const TAttLine*>(this),
		 // dynamic_cast<const TAttFill*>(this),
		 // dynamic_cast<const TAttMarker*>(this),
		 subsub);
    out << "\n"
	<< sub << "},\n";

    out << sub << JSON::k("sum-attr") << " {\n";
    JSON::outatt(out,fSumOption, fSumTitle, &fSumLine, &fSumFill,
		 0, subsub);
    out << "\n"
	<< sub << "},\n";

    out << sub << JSON::k("common-sum-attr") << " {\n";
    JSON::outatt(out,fCommonSumOption, fCommonSumTitle,
		 &fCommonSumLine, &fCommonSumFill,
		 0, subsub);
    out << "\n"
	<< sub << "},\n";

    JSON::outmap(out,fQualifiers, "qualifiers", sub);
    out << ",\n";
    JSON::outmap(out,fMap, "keywords", sub);
    out << ",\n";

    out << sub << JSON::k("common") << " {";
    JSON::outsys(out, &fCommon, JSON::cmn, subsub);
    out << sub << "},\n";
    out << sub << JSON::k("point-to-point") << " {";
    JSON::outsys(out, &fPoint2Point, 0, subsub);
    out << sub << "},\n";
    ...
    std::transform(i.begin(),i.end(),v.begin(),
		   [this](int i) { return this->GetX(i); });
    JSON::outdat(out,"x",v,subsub,false);

    std::transform(i.begin(),i.end(),v.begin(),
		   [this](int i) { return this->GetErrorXLeft(i); });
    JSON::outdat(out,"exl",v,subsub);

    std::transform(i.begin(),i.end(),v.begin(),
		   [this](int i) { return this->GetErrorXRight(i); });
    JSON::outdat(out,"exh",v,subsub);

    std::transform(i.begin(),i.end(),v.begin(),
		   [this](int i) { return this->GetY(i); });
    JSON::outdat(out,"y",v,subsub);

    std::transform(i.begin(),i.end(),v.begin(),
		   [this](int i) { return this->GetStatErrorDown(i); });
    JSON::outdat(out,"syl",v,subsub);

    std::transform(i.begin(),i.end(),v.begin(),
		   [this](int i) { return this->GetStatErrorUp(i); });
    JSON::outdat(out,"syh",v,subsub);

    const char* subsubsub = (std::string(subsub)+"  ").c_str();
    HolderP2P*  p2p   = 0;
    TIter       nextP(&fPoint2Point);
    while ((p2p = static_cast<HolderP2P*>(nextP()))) {
      out << ",\n"
	  << subsub << JSON::k(p2p->GetName()) << " {";

      std::transform(i.begin(),i.end(),v.begin(),
		     [p2p](int i) { return p2p->GetXLeft(i); });
      JSON::outdat(out,"exl",v,subsubsub,false);

      std::transform(i.begin(),i.end(),v.begin(),
		     [p2p](int i) { return p2p->GetXRight(i); });
      JSON::outdat(out,"exh",v,subsubsub);

      std::transform(i.begin(),i.end(),v.begin(),
		     [p2p](int i) { return p2p->GetYDown(i); });
      JSON::outdat(out,"eyl",v,subsubsub);

      std::transform(i.begin(),i.end(),v.begin(),
		     [p2p](int i) { return p2p->GetYUp(i); });
      JSON::outdat(out,"eyh",v,subsubsub);
    ...
    for (Int_t i = 0; i < n; ++i) {
      ...
      Double_t y   = GetY(i);
      Double_t syl = GetStatErrorDown(i);
      Double_t syh = GetStatErrorUp(i);
  ...
  void RemoveSysError(Int_t id)
  {
    ...
    if (h) {
      // Printf(" Got common: %s", h->GetTitle());
      fCommon.Remove(h);
      return;
    }
  ...
  { 
    ...
    while ((holder = static_cast<Holder*>(nextC()))) {
      Bool_t match = tit.EqualTo(holder->GetTitle());
#if 0
      Printf("Got common sys='%s' [%d] looking for %s, match? %s",
	     holder->GetTitle(), holder->GetUniqueID(),
	     tit.Data(),match ? "yes" : "no");
#endif 
      if (match) return holder->GetUniqueID();
    }
    ...
    while ((holder = static_cast<Holder*>(nextP()))) {
      Bool_t match = tit.EqualTo(holder->GetTitle());
#if 0
      Printf("Got p2p sys='%s' [%d] looking for %s, match? %s",
	     holder->GetTitle(), holder->GetUniqueID(),
	     tit.Data(),match ? "yes" : "no");
#endif 
      if (match) return holder->GetUniqueID();
  ...
  /** 
   * Get the current value of the systematic error counter.  Note,
   * this is @e not quarantied to be the actual number of systematic
   * uncertainties defined.
   * 
   * @return Current value of the systematic error counter
   */
  ...
  void SetSysFraction(Int_t id, Double_t f=1)
  {
    Holder* h = Find(id);
    if (!h) return;
    h->SetFraction(f);
  }
  void ResetSysFraction(Int_t id)
  {
    Holder* h = Find(id);
    if (!h) return;
    h->ResetFraction();
		 ...
		 Int_t&    imin, Int_t&    imax) const
  {
    ...
    ymax = TMath::Limits<Double_t>::Min();
    xmin = TMath::Limits<Double_t>::Min();
    ...
    for (Int_t i = 0; i < GetN(); i++) {
      Double_t eyl = 0;
      Double_t eyh = 0;
      Double_t y   = (noerr ? GetY(i) :
		      GetYandError(i, cmn, stat, quad, false, eyl, eyh));
      Double_t yl  = y - eyl;
      Double_t yh  = y + eyh;
      if (yl < ymin) {
	ymin = yl;
  ...
  void SetKey(const char* key, const char* value, Bool_t replace=false) //*MENU*
  {
    ...
    v = t;
    if (k.EqualTo("caption")) k = "dscomment";
    ...
    if (k.EqualTo("comment", TString::kIgnoreCase)) { 
      ...
      if (!l.IsNull() && !a.IsNull() && v.BeginsWith(l))
  ...
  /** 
   * Remove a key.  If the key have multiple values, one can remove
   * all of them or only a sub-set specified by an @a offset and a
   * count @a n.
   * 
   * @param key     Key to remove value for 
   * @param offset  At which offset we need to remove the values.  If
   *                negative, remove all values
   * @param n       How many values to remove.  If negative, remove all
   *                values after @a offset.
   * 
   * @return true if any value was removed. 
   */
  Bool_t RemoveKey(const char* key, Int_t offset=-1, Int_t n=-1)
  {
    if (!fMap) return false;
    Int_t    cnt = 0;
    Int_t    dne = 0;
    TObject* o   = 0;
    while ((o = fMap->FindObject(key))) {
      cnt++;
   ...
   *
   * @code 
   g.AddQualifier("RE","P PB --> CHARGED X");
   g.AddQualifier("SQRT(S)/NUCLEON in GEV","5020.0")
   @endcode 
   * @param key     The key 
   ...
   * @param seyl   On return low error on y (with statistical errors)
   * @param seyh   On return high error on y (with statistical errors)
   * 
  ...
  struct Combiner : public TObject
  {
	...
	Printf("%10.8f  -%10.8f +%10.8f", fX, fEl, fEh);
      ...
      ClassDef(Observation,2); // An experimental observation 
    };
	...
	Printf("%10.8f  -%10.8f +%10.8f  %5.2f", fX, fEl, fEh, fChi2);
      ...
      ClassDef(Result,2); // Final result 
    };
     ...
     *
     * - DATA    Clear the internal data (default)
     * - COMMON  Remove all common systematic uncertainties 
     * - P2P     Remove all point-to-point systematic uncertainties 
     * - SYS     Remove all systematic uncertainties 
     */
    ...
    ClassDef(Combiner,2); // Combine systematics 
  };
  ...
  struct LinearSigmaCombiner : public Combiner
  {
    ...
    ClassDef(LinearSigmaCombiner,2); // Sigma combiner 
  };
  /**
  ...
  struct LinearVarianceCombiner : public Combiner
  {
     ...
     */
    Wrapper_t Wrapper() const { return WrapL;  }
    ClassDef(LinearVarianceCombiner,2); // Variance combiner 
  };
   ...
   */
  struct __attribute__ ((visibility("default"))) Holder : public TNamed, TAttLine, TAttFill 
  {
    /** Containing class is a friemd */
    friend struct GraphSysErr;
    // friend struct GraphSysErr::JSON;
    /** 
     * CTOR
     */
    Holder() 
  ...
  protected:
    ...
    /** 
     * Set fraction of errors to use 
     *
     * @param f Fraction 
     */
    void SetFraction(Double_t f=1) { fFraction = f; }
    /** 
     * Reset fraction 
     */
    void ResetFraction() { fFraction = 1; }
    ...
    {
      gROOT->IndentLevel();
      Printf("%s/%s %s(ID # %d%s %3.1f)", GetName(), GetTitle(),
	     IsRelative() ? "PCT " : "",  GetUniqueID(),
	     TestBit(kUsedBit) ? " used" : "", fFraction);
      TString opt(option);
  ...
  protected:
    ...
    {
      if (!fGraph) return 0;
      return fFraction*fGraph->GetErrorYlow(point);
    }
    /** 
    ...
    {
      if (!fGraph) return 0;
      return fFraction*fGraph->GetErrorYhigh(point);
    }
    /* @} */
		  ...
		  Double_t& eyh) const
    {
      ...
      Double_t eylO = fFraction*fGraph->GetErrorYlow(i);
      Double_t eyhO = fFraction*fGraph->GetErrorYhigh(i);
      Double_t oyl  = eylO;
      Double_t oyh  = eyhO;
      if (exl <= 0) exlO = exl;
      if (exh <= 0) exhO = exh;
	...
	ga->SetPointError(i, exl,exh,eyl,eyh);
      ...
      ga->SetName(Form("%s_stack_%08x", g->GetName(), GetUniqueID()));
      return ga;
    }
    /** 
     * Sum up point errors 
    ...
    {
      gROOT->IndentLevel();
      Printf("%s/%s (ID # %d%s%s, %d points, x%3.1f)",
	     GetName(), GetTitle(), GetUniqueID(),
	     TestBit(kUsedBit) ? " used" : " ready", 
	     TestBit(kOnlyWeightBit) ? " weighs" : " full",
   ...
   * 
   */
  struct __attribute__ ((visibility("default"))) HolderCommon : public Holder 
  {
    /** Containing class is a friemd */
    friend struct GraphSysErr;
    /** 
  ...
  protected:
    ...
    {
      return fFraction*(fRelative ? y : 1) * fEyl;
    }
    /** 
     * Get the up error 
     * 
    ...
    {
      return fFraction*(fRelative ? y : 1) * fEyh;
    }
    /* @} */
    ...
    virtual void Print(Option_t* option="") const
    {
      ...
      const char* pst = (rel ? "%" : "");
      gROOT->IndentLevel();
      Printf("%s/%s (ID # %d) x%3.1f -%f%s +%f%s",
	     GetName(), GetTitle(), GetUniqueID(),
	     fFraction, fac*fEyl, pst, fac*fEyh, pst);
      TString opt(option);
      if (!opt.Contains("ATTR", TString::kIgnoreCase)) return;
      gROOT->IndentLevel();
      Printf(" [option: %3d line (c/s/w):%3d/%1d/%2d fill (c/s):%3d/%4d]",
    ...
    ClassDef(HolderCommon,4);
  };
  struct JSON {
    static std::string e(std::string s)
    {
      size_t start_pos = 0;
    ...
    static const char* k(const char* key)
    {
      return Form("\"%s\":",e(key).c_str());
    }
    static const char* s(const char* val)
    {
      return Form("\"%s\"",e(val).c_str());
    }
    static const char* getcol(Int_t num)
    {
      return gROOT->GetColor(num)->AsHexString();
    }
    static const char* getls(Int_t num)
    {
      switch (num) {
      case 1: return "solid";
      ...
      case 5: return "dashdotted";
      case 6: return "dashdotdotted";
      case 7: return "loosely dashed";
      case 8: return "loosely dashdotdotted";
      case 9: return "densely dashed";
      case 10: return "loosely dashdotted";
      }
      return "solid";
    }	  
    static const char* getfs(Int_t style)
    {
      switch (style) {
      case 0:    return "none";
      case 1001: return "solid";
      case 3001: case 3002: case 3003: return ".";
      case 3144: case 3244: case 3344: case 3444: case 3544:
      case 3644: case 3744: case 3844: case 3944: return "x";
      case 3305: case 3350: case 3007: return "-";
      case 3395: case 3359: case 3006: return "|";
      case 3315: case 3325: case 3335: case 3345: case 3365:
      case 3375: case 3385: case 3005: return "\\";
      case 3351: case 3352: case 3353: case 3354: case 3356:
      case 3357: case 3358: case 3004: return "/";
      case 3021: case 3022: case 3023: case 3024: case 3025: return "*";
      case 3016: case 3017: case 3018: case 3019: case 3020: return "O";
      case 3011: case 3012: case 3013: case 3014: case 3015: return "o";
      }
      return "solid";
    }
    static const char* getms(Int_t style)
    {
      switch (style) {
      case 0: return "none";
      case 1: return ",";
      case 7: case 8: return ".";
      case 2: case 31: return "+";
      case 3: case 29: case 30: return "*";
      case 4: return "o";
      case 5: return "x";
      case 9: case 10: case 11: case 12: case 13: case 14:
      case 15: case 16: case 17: case 18: case 19: case 20:
      case 24: return "o";
      case 21: case 25: return "s";
      case 22: case 26: return "^";
      case 23: case 32: return "v";
      case 27: case 33: return "d";
      case 28: case 34: return "P";
      case 35: return "D";
      case 37: case 39: return "1";
      case 40: case 41: return "h";
      case 38: return "2";
      case 42: case 43: return "H";
      }
      return "o";
    }
    static const char* getmf(Int_t style)
    {
      switch (style) {
      case 4: case 24: case 25: case 26: case 27: case 28:
      case 30: case 32: case 37: case 40: case 42:
	return "none";
      }
      return "full";
    }
    static const char* getopt(UInt_t option)
    {
      switch (option) {
      case 0:  return "ticks";        // Line with ticks
      case 1:  return "no-ticks";     // Line with no ticks 
      case 2:  return "arrows";       // Linw with arrows 
      case 3:  return "filled";       // Rectangle w/fill
      case 4:  return "filled-line";  // Rectangle w/fill & line 
      case 5:  return "fill-between"; // Filled area 
      case 6:  return "fill-between"; // Filled smoothed area 
      case 7:  return "hats";	    // Hats 
      case 8:  return "bars";	    // A bar 
      case 9:  return "none";	    // No errors
      case 10: return "line";        
      case 11: return "line";        
      }
      return "ticks";
    ...
    static void outatt(std::ostream&     out,
		       UInt_t            option,
		       const char*       title,
		       const TAttLine*   line=0,
		       const TAttFill*   fill=0,
		       const TAttMarker* marker=0,
		       const char*       ind="    ")
    {
      out << ind << k("option") << " " << s(getopt(option)) << ",\n"
	  << ind << k("title") << "  " << s(title);
      if (line)
	out << ",\n"
	    << ind << k("line") << " {\n"
	    << ind << "  " << k("color") << " "
	    << s(getcol(line->GetLineColor())) << ",\n"
	    << ind << "  " << k("style") << " "
	    << s(getls(line->GetLineStyle())) << ",\n"
	    << ind << "  " << k("width") << " "
	    << line->GetLineWidth() << "\n"
	    << ind << "}";
      if (fill)
	out << ",\n" 
	    << ind << k("fill") << " {\n"
	    << ind << "  " << k("color") << " "
	    << s(getcol(fill->GetFillColor())) << ",\n"
	    << ind << "  " << k("style") << " "
	    << s(getfs(fill->GetFillStyle())) << "\n"
	    << ind << "}";
      if (marker)
	out << ",\n"
	    << ind << k("marker") << " {\n"
	    << ind << "  " << k("color") << " "
	    << s(getcol(marker->GetMarkerColor())) << ",\n"
	    << ind << "  " << k("style") << " "
	    << s(getms(marker->GetMarkerStyle())) << ",\n"
	    << ind << "  " << k("fill") << "  "
	    << s(getmf(marker->GetMarkerStyle())) << "\n"
	    << ind << "}";
    }
    static void outmap(std::ostream& out, TCollection* c, const char* name,
		       const char* ind="  ")
    {
      out << ind << k(name)  << " {";
      bool comma = false;
	...
	const char* val = obj->GetTitle();
	if (comma) out << ",";
	out << "\n" << ind << "  " << k(key) << " "
	    << (!val or val[0]=='\0' ? "true" : s(val));
	comma = true;
      }
      out << "\n" << ind << "}";
    }
    static void outsys(std::ostream& out, TCollection* c,
		       void (*extra)(std::ostream& o, Holder* h,
				     const char* ind),
		       const char* ind="    ")
    {
      TIter   next(c);
      ...
      bool comma = false;
      const char* sub = (std::string(ind) + "  ").c_str();
      while ((hld = static_cast<Holder*>(next()))) {
	if (comma) out << ",";
      
	out << "\n"
	    << ind << k(hld->GetName()) << " {\n"
	    << ind << "  " << k("relative") << " "
	    << (hld->IsRelative()?"true":"false")
	    << ",\n";
	JSON::outatt(out,hld->GetDOption(), hld->GetTitle(),
		     static_cast<const TAttLine*>(hld),
		     static_cast<const TAttFill*>(hld),
		     0, sub);
	if (extra) {
	  (*extra)(out,hld,sub);
	}
    ...
    static void cmn(std::ostream& out, Holder* hld, const char* ind)
    {
      HolderCommon* c = dynamic_cast<HolderCommon*>(hld);
      if (!c) return;

      out << ",\n"
	  << ind << k("down") << "     " << c->GetYDown(1) << ",\n"
	  << ind << k("up") << "       " << c->GetYUp(1);
    }
    static void outdat(std::ostream& out,
		       const char* name,
			...
			Double_t& wyh) const
  {
    ...
    while ((hp = static_cast<HolderP2P*>(nextP()))) {
      if (hp->TestBit(kUsedBit)) {
	// Printf("Ignoring %s/%s for weight value",
	//        GetTitle(), hp->GetTitle());
	continue;
      }

      if (xMode < 0) xMode = hp->XMode(fSumOption);	
      hp->SumPointError(i, xMode, false, quad, exl, exh, wyl, wyh);
      // Printf("Adding sys %s/%s: %f %f to weights -> %f, %f",
      // GetTitle(), hp->GetTitle(), hp->GetYDown(i), hp->GetYUp(i),
      // wyl, wyh);

      if (hp->TestBit(kOnlyWeightBit)) {
	//Printf("Ignoring %s/%s for error value", GetTitle(), hp->GetTitle());
	continue;	
      }

      hp->SumPointError(i, xMode, false, quad, exl, exh, eyl, eyh);
      // Printf("Adding sys %s/%s: %f %f to errors -> %f, %f",
      // GetTitle(), hp->GetTitle(), hp->GetYDown(i), hp->GetYUp(i),
      // eyl, eyh);
    }
    if (quad && !nosqrt) {
  ...
  /** 
   * Get the weighted average in Y 
   *
   * 
   */
  Double_t GetYAverage(Double_t& el, Double_t& eh) const
  {
    // Use our combiner framework for calculating the averaged
    // result and errors.  Note, we define two combiners.  One that
    // include all non-shared common and point-to-point errors
    ...
    LinearSigmaCombiner stats;
    for (Int_t i = 0; i < GetN(); i++) {
      Bool_t cmn    = false; // No commons 
      Bool_t stat   = true;  // Include stats
      Bool_t quad   = true;
      Bool_t nosqrt = false;
      Double_t eyl, eyh, wyl, wyh;
      Double_t y   = GetYandError(i,cmn,stat,quad,nosqrt,eyl,eyh,wyl,wyh);
      Double_t syl = GetStatErrorDown(i);
      Double_t syh = GetStatErrorUp(i);
      centroid.Add(y,eyl,eyh);
    }
    centroid.Calculate();
    el = centroid.fResult->fEl;
    eh = centroid.fResult->fEh;
    return centroid.fResult->fX;
  }
  /** 
   ...
   * @return The "mantisa"
   */
  static Double_t Round(Double_t v, Int_t p, Int_t& rexpo,bool adjust=true)
  {
    if (v == 0) {
      // ret.Form("%.*f", p-1, 0);
      rexpo = 0;
    ...
    Double_t w    = RoundN(tens, tmp); // TMath::Ceil(tmp/tens);
    // Printf("%f,%d -> tmp=%f\texpo=%d\ttens=%e\tw=%f",v,p,tmp,expo,tens,w);

    if (w < TMath::Power(10, p-1) and adjust) {
      Double_t w1 = w;
      // If scaled is less than 10 to our precision, take off one digit
      expo--;
      tens = TMath::Power(10, expo - p + 1);
      w    = RoundN(tens,tmp); // TMath::Ceil(tmp/tens);
      // Printf("%f<%f -> expo=%d\ttens=%e\tw=%f",w1,TMath::Power(10,p-1),expo,tens,w);
    }


    if (TMath::Abs((w+1) * tens) <= TMath::Abs(w * tens))  {
      // If the value is not near original, add one
      Double_t w1 = w;
      w++;
      // Printf("%f <= %f w=%f",TMath::Abs((w+1)*tens),TMath::Abs(w*tens),w);
    }
    if (w >= TMath::Power(10,p)) {
   ...
   */
  static Double_t RoundN(Double_t tens, Double_t tmp, Int_t debug=0)
  {
    // Add small number to not loose least digit if a 1
    Double_t w     = TMath::Floor(100*tmp/tens+.00001);
    if (debug>1) Printf("tens=%g, tmp=%g (%g) -> w=%g",
			tens, tmp, tmp/tens, w);
    Int_t    next  = int(w/10) % 10;
    if (debug>1) Printf("Check next=%d", next);
    if (next > 5) 
      // Round up 
      return TMath::Ceil(w/100);
    if (next < 5)
      // Round down
      return TMath::Floor(w/100);
    Int_t nnext = int(w) % 10;
    if (debug>1) Printf("Check next to next=%d", nnext);
    if (nnext != 0)
      // Round up 
      return TMath::Ceil(w/100);
    Int_t last = int(w/100) % 10;
    if (debug>1) Printf("Check last=%d", last);
    if (last % 2 == 0)
      // Round down
      return TMath::Floor(w/100);
...
protected:
  /** 
   * Interchange to points on the graph. 
   * 
   * @param g        Graph to manipulate 
   * @param i        First point
   * @param j        Second point
   * @param reflect  If true, also invert sign on X coordinate
   */
			     ...
			     Int_t  nsign=-1) const
  {
	...
	ExportKey(out, "reference", true, "<reference>");
      ...
      out << FormatKey("detector") << (exper ? exper : "<experiment>") << '\n'
	  << FormatKey("experiment") << (lab ? lab : "<lab>") << '-' 
	  << (accel ? accel : "<accelerator>") << '-' 
	  << (exper ? exper : "<experiment>") << std::endl;
      ExportKey(out, "title",     true, "<Title of paper>");      
      out << FormatKey("comment")  << (lab ? lab : "<lab>") << '-' 
	  << (accel ? accel : "<accelerator>") << ". " 
	  << (abs ? abs : "<abstract>") << "\n"
	  << std::endl;
    }
  ...
  /** 
   * Figure out the least exponent to use for an error 
   * 
   * @param low  Low error 
   ...
   */
  static Int_t LeastExponent(Double_t low,
			     Double_t high,
			     Bool_t   rel,
			     Int_t    nsign)
  {
    //::Info("LeastExponent", "l=%f,h=%f,rel=%d,nsign=%d",low,high,rel,nsign);
    if (nsign < 0) return 9999;
    Bool_t   n = (rel ?
		  (TMath::Abs(low) > 1e-5 || TMath::Abs(high) > 1e-5) :
		  (TMath::Abs(low) > 1e-9 || TMath::Abs(high) > 1e-9));
    if (!n) {
#if 0
      ::Info("LeastExponent",
	     "Insignificant errors |l|=%f<=1e-%d |h|=%f<=1e-%d",
	     TMath::Abs(low),  rel ? 5 : 9,
	     TMath::Abs(high), rel ? 5 : 9);
#endif 
      return 9999;
    }
    
    Int_t    le, he;
    Double_t lm = Round(low,  nsign, le);
    Double_t hm = Round(high, nsign, he);
    Int_t    re = TMath::Min(le, he);
    // ::Info("LeastExponent", "Got for low=%d for high=%d -> %d", le, he, re);
    return re;
  }
  /** 
   * Export an error 
   ...
   * @param rel   If true, the error is relative 
   * @param n     number of significant digits/Least exponent to use
   * @param nIsLe If true, n is least exponent, 
   *              otherwise number of significant digits. 
   * 
   * @return Least exponent used
   */
			   ...
			   Bool_t   nIsLe=false)
  {
    ...
    Bool_t   nz = (rel ?
		  (TMath::Abs(low) > 1e-5 || TMath::Abs(high) > 1e-5) :
		  (TMath::Abs(low) > 1e-9 || TMath::Abs(high) > 1e-9));
    Int_t    el = Int_t(TMath::Log10(low));  // if (el>0) el++;
    Int_t    eh = Int_t(TMath::Log10(high)); // if (eh>0) eh++;
    Int_t    nl = nIsLe ? el - n + 1: n;
    Int_t    nh = nIsLe ? eh - n + 1: n;
    // Printf("el=%d eh=%d n=%d nl=%d nh=%d",el,eh,n,nl,nh);
    if (nl >= 0 && nh >= 0 && nz) {
      Int_t    pp = nIsLe ? p-n : p;
      Int_t    le, he;
      Double_t lm = Round(l, nl, le, not nIsLe);
      Double_t hm = Round(h, nh, he, not nIsLe);
      e           = TMath::Min(le,he);
      l           = lm*TMath::Power(10,le);
      h           = hm*TMath::Power(10,he);
      o.precision(pp);
#if 0
      Printf("Exporting error %f,%f -> (%3d,%3d:%3d) -> %f(%d),%f(%d) -> %f,%f (%d,%d)",
	     low, high, nl, nh, n, lm, le, hm, he, l, h, p, pp);
#endif 
    }
    else
      e = 9999;
			    ...
			    Int_t         nxsign=0) const
  {
	...
	if (TMath::Abs(exh-exl) < 1e-10) {
	  ...
	  if (nxsign <= 0)
	    ...
	    Int_t    mp = LeastExponent(lx,hx,false,nxsign);
	    Int_t    nl = Int_t(TMath::Log10(TMath::Abs(lx)))-mp+1;
	    Int_t    nh = Int_t(TMath::Log10(TMath::Abs(hx)))-mp+1;
	    Int_t le, he;
	    Double_t ml = Round(lx,nl,le,false);
	    Double_t mh = Round(hx,nh,he,false);
	    Double_t ll = ml*TMath::Power(10,le);
	    Double_t hh = mh*TMath::Power(10,he);
	    Int_t    nx = nxsign - mp;
	    out.precision(nx);
	    out << ' ' << ll << " TO " << hh;
	  }
	  ...
	  if (nxsign <= 0) 
	    ...
	    //  out.precision(pp);
	    // out << '+' << exh << ",-" << exl;
	    Int_t    mp = LeastExponent(exl,exh,false,nxsign);
	    Int_t    nn = Int_t(TMath::Log10(x))-mp+1;
	    Int_t    px;
	    Double_t mx = Round(x, nn, px, false);
	    Double_t xx = mx*TMath::Power(10,px);
	    Int_t    nx = nxsign - mp;
	    out.precision(nx);
	    out << ' ' << x << ' ';
	    out.precision(nxsign);
	    ExportError(out,exl,exh,true,false,mp,true);
	  }
	}
      }
      out.precision(p);
    ...
    Double_t seyl    = GetStatErrorDown(i) * fy;
    Double_t seyh    = GetStatErrorUp(i)   * fy;

    // Now we need to figure out the least exponent
    Int_t    le = LeastExponent(seyl, seyh, statRel, nsign);
    // Int_t le = TMath::Min(l1,l2); 
    // Printf("Least exponent for stat (-%f,+%f) -> %d (%d)",
    //        seyl, seyh, le, nsign);
	...
	Int_t    ell = LeastExponent(esl, esh, rel, nsign);
	// Printf("Least exponent from %s (%f,%f)-> %d -> %d (was %d)",
	//        holderP2P->GetName(), esl, esh,ell, TMath::Min(le,ell), le);
	le           = TMath::Min(le, ell);
      }
    }
    // Printf("Least exponent: %d (nsign=%d)", le, nsign);
    // out << y << ' ';
    std::stringstream tmp;
    tmp.flags(out.flags());
    tmp.precision(nsign);
    ExportError(tmp, seyl, seyh, false, statRel, le, true);
	...
	Double_t esl = holderP2P->GetYDown(i)*fy;
	tmp << "DSYS=";
	ExportError(tmp, esl, esh, true, rel, le, true);
	if (sysName) 
    ...
    if (nsign) {
      Int_t    ll = (le >= 999 ? 0 : le-1);
      Int_t    lY = Int_t(TMath::Log10(TMath::Abs(y)));
      Int_t    nn = lY-ll; // if (nn >= 0) nn++;
      Double_t yy = y;
      Int_t    nY = TMath::Ceil(lY)-ll;
      Int_t    pp = nsign;
      if (le < 999) {
	yy = Round(y, nn, nY)*TMath::Power(10, nY);
	pp = (le < 0 ? -le : 0);
      }
      // Printf("Roudning %f to %2d (%2d,%2d) -> %f (%d)", y, nn, nsign, ll, yy, pp);
      // out.precision(pp);
      out << yy << ' ';
      out.precision(p);
    }
  ...
  Bool_t CheckToggle(const TString& opt,
		     const char*    trueVal,
		     const char*    falseVal,
		     Bool_t         def)
  {
    Bool_t trueOK  = opt.Contains(trueVal);
    Bool_t falseOK = opt.Contains(falseVal);
    if (trueOK && !falseOK) return true;
    if (falseOK)            return false;
    if (opt.Contains(trueVal))  return true;
    if (opt.Contains(falseVal)) return false;
    return def;
  }
  /** 
   * Make our stack 
  ...
  TMultiGraph* MakeMulti(Option_t* option)
  {
    ...
    // Info("MakeMulti", "Making multi with %s", option);
    ...
    Bool_t   cmn      = opt.Contains("COMMON");
    Bool_t   combine  = CheckToggle(opt,"COMBINED","STACK",true);
    Bool_t   stat     = opt.Contains("STAT");
    Bool_t   quad     = CheckToggle(opt, "QUAD", "DIRECT", true);
    Bool_t   split    = opt.Contains("SPLIT");
    Int_t    xpos     = CheckToggle(opt,"EAST","WEST",true) ? 1 : -1;
    ...
    if (opt.Contains("YBASE=")) {
      Int_t   idx = opt.Index("YBASE=")+6;
      TString tmp = opt(idx,opt.Length()-idx);
      Bool_t  at  = tmp[0] == '@';
      if (at) {
	TString  tmpp = tmp(1,tmp.Length()-1);
	Double_t xat  = tmpp.Atof();
	Double_t y, eyl, eyh;
	if (FindYandError(xat, false,true,true,false,y,eyl,eyh)) {
	  yBase = y;
	}
      }
      else
	...
	Double_t y = fData->GetY()[i];
	switch(ypos) {
	case -1: yBase =  TMath::Min(yBase, y); break;
	case 0:  yBase += y;                    break;
	case +1: yBase =  TMath::Max(yBase, y); break;
	}
      }
      if (ypos == 0) yBase /= n;
    }
    // Printf("yBase=%f",yBase);
	...
	if (cmn) { 
	  ...
	  while ((h = static_cast<Holder*>(nextC()))) {
	    // if (i == 0) Printf("Adding %s to combined error",h->GetTitle());
	...
	while ((hc = static_cast<HolderCommon*>(nextC()))) {
	  ...
	  if (!prev)
	    ...
	    // New ROOT6 f*cks up the previous access to the list
	    // options. We have no way of adding an object before
	    // another _with_ opytions, which is really f*cked up.
	    drawn.AddBefore(prev, g);
	    // FormatOption(hc->GetDOption())
#else 
	    // Here, we use the underlying linked list of the list so
	...
	if (fCommon.GetEntries() > 0) {
	  ...
	  g->SetMarkerSize(0);
	  g->SetMarkerColor(0);
	  g->SetTitle(fCommonSumTitle);
	  g->SetName("combinedCommon");
