...
class GraphSysErr : public TNamed, public TAttMarker,
 ...
 public:
    ...
    if (sep) {
      ...
      while ((other = static_cast<GraphSysErr *>(oNext()))) {
        ...
        while ((p2p = static_cast<HolderP2P *>(oNextP())) && off < 64) {
          ...
          // in the result.
          if (verb)
            Printf("Mark %s/%s for weights only", other->GetTitle(),
                   p2p->GetTitle());
    ...
    for (Int_t i = 0; i < ndf; i++) {
      switch (type) {
        ...
        case kExperimentTruth: {
          ...
          break;
        default: ::Warning("ChisquareTest", "Should not happen");
          return -1;
      }
  ...
  void Export(std::ostream &out = std::cout,
    ...
    TString xTit = fXTitle;
    EscapeLtx(xTit, "fill");
    TString yTit = fYTitle;
    EscapeLtx(yTit, "fill");
  ...
  static GraphSysErr *Import(std::istream &in, UShort_t idx = 0,
    ...
    do {
      ...
      tmp = xCol.Strip(TString::kBoth, ' ');
      xCol = tmp;
      tmp = yCol.Strip(TString::kBoth, ' ');
      yCol = tmp;
      TString yFull = yCol;
  ...
  void SetKey(const char *key, const char *value, Bool_t replace = false) //*MENU*
  {
    ...
    if (k.EqualTo("experiment", TString::kIgnoreCase)) {
      ...
      t = Token(l, 0);
      TString lab = t.Strip(TString::kBoth);
      t = Token(l, 1);
      TString acc = t.Strip(TString::kBoth);
      t = Token(l, 2);
      TString exp = t.Strip(TString::kBoth);
 ...
 protected:
  ...
  const char *FormatOption(UInt_t opt) {
    ...
    switch (opt) {
      case kNormal: ret = "";
        break; // Line with ticks
      case kNoTick: ret = "Z0";
        break; // Line with no ticks
      case kArrow: ret = ">0";
        break; // Linw with arrows
      case kRect: ret = "20";
        break; // Rectangle w/fill
      case kBox: ret = "50";
        break; // Rectangle w/fill & line
      case kFill: ret = "30";
        break; // Filled area
      case kCurve: ret = "40";
        break; // Filled smoothed area
      case kHat: ret = "[]0";
        break; // Hats
      case kBar: ret = "||0";
        break; // A bar
  ...
  TMultiGraph *MakeMulti(Option_t *option) {
    ...
    if (opt.Contains("YBASE=")) {
      ...
      for (Int_t i = 0; i < n; i++) {
        Double_t y = fData->GetY()[i];
        switch (ypos) {
          case -1: yBase = TMath::Min(yBase, y);
            break;
          case 0: yBase += y;
            break;
          case +1: yBase = TMath::Max(yBase, y);
            break;
        }
      }
      if (ypos == 0) yBase /= n;
